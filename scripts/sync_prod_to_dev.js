// Sync production workout changes to dev database
const { initDatabase, Database } = require('../database');

async function syncProdToDev() {
  console.log('════════════════════════════════════════════════');
  console.log('🔄 Syncing Production Changes to Dev');
  console.log('════════════════════════════════════════════════');
  console.log('');
  ...
  try {
    ...
    async function findExId(name) {
      const result = await devDb.query(`SELECT id FROM exercises WHERE LOWER(name) = LOWER(?) LIMIT 1`, [name]);
      return result.rows.length > 0 ? result.rows[0].id : null;
    }

    // Helper to find equipment ID
    async function findEqId(name) {
      const result = await devDb.query(`SELECT id FROM equipment WHERE LOWER(name) = LOWER(?) LIMIT 1`, [name]);
      return result.rows.length > 0 ? result.rows[0].id : null;
    }
    ...
    async function replaceInBlock(workoutId, blockType, oldName, newName, equipment = null) {
      ...
      if (!newExId) {
        console.log(`   ⚠️  Exercise "${newName}" not found in dev - creating it`);
        // Create the exercise
        const createResult = await devDb.query(`
          INSERT INTO exercises (name, description, created_at, updated_at)
          VALUES (?, NULL, datetime('now'), datetime('now'))
        `, [newName]);
        const newId = await findExId(newName);
        ...
        if (equipment) {
          ...
          if (eqId) {
            await devDb.query(`DELETE FROM exercise_equipment WHERE exercise_id = ?`, [newId]);
            await devDb.query(`INSERT OR IGNORE INTO exercise_equipment (exercise_id, equipment_id) VALUES (?, ?)`, [newId, eqId]);
          }
        }
        await devDb.query(`UPDATE block_exercises SET exercise_id = ? WHERE id = ?`, [newId, oldEx.rows[0].id]);
        return true;
      }
      ...
      if (equipment) {
        ...
        const eqId = await findEqId(eqName);
        if (eqId) {
          await devDb.query(`DELETE FROM exercise_equipment WHERE exercise_id = ?`, [newExId]);
          await devDb.query(`INSERT OR IGNORE INTO exercise_equipment (exercise_id, equipment_id) VALUES (?, ?)`, [newExId, eqId]);
        }
      }
    ...
    async function addToBlock(workoutId, blockType, exerciseName, equipment = null, orderIndex = null) {
      ...
      let exId = await findExId(exerciseName);
      if (!exId) {
        // Create the exercise
        await devDb.query(`
          INSERT INTO exercises (name, description, created_at, updated_at)
          VALUES (?, NULL, datetime('now'), datetime('now'))
        `, [exerciseName]);
        exId = await findExId(exerciseName);
      ...
      if (equipment) {
        ...
        const eqId = await findEqId(eqName);
        if (eqId) {
          await devDb.query(`DELETE FROM exercise_equipment WHERE exercise_id = ?`, [exId]);
          await devDb.query(`INSERT OR IGNORE INTO exercise_equipment (exercise_id, equipment_id) VALUES (?, ?)`, [exId, eqId]);
        }
      }
